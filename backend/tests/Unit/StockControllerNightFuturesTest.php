...
class StockControllerNightFuturesTest extends TestCase
{
     ...
     */
    public function testNqSessionFollowsCmeEtBoundaries(string $etLiteral, string $expected, string $why): void
    {
     ...
     */
    public function etBoundaryProvider(): array
    {
        return [
            // ── 여름 EDT ────────────────────────────────────────────────
     ...
     */
    public function testNqSessionKstEquivalenceAcrossDst(string $kstLiteral, string $expected, string $why): void
    {
    ...
    public function kstEquivalenceProvider(): array
    {
        return [
            // ── 겨울 EST: 금 17:00 ET = 토 07:00 KST · 일 18:00 ET = 월 08:00 KST ──
            'EST 토 06:30 KST = 금 16:30 ET → 거래중'
                => ['2026-01-17 06:30', '거래중', 'EST 토 06:30 KST 는 금 16:30 ET — 아직 거래중. 옛 KST 하드코딩(토 06:00~)은 여기서 장마감으로 오판했다.'],
            'EST 토 07:00 KST = 금 17:00 ET → 장마감(마감 경계)'
                => ['2026-01-17 07:00', '장마감', 'EST 마감 경계는 토 06:00 이 아니라 토 07:00 KST 다.'],
            'EST 월 07:30 KST = 일 17:30 ET → 장마감'
                => ['2026-01-19 07:30', '장마감', 'EST 월 07:30 KST 는 일 17:30 ET — 아직 휴장. 옛 KST 하드코딩(~월 07:00)은 여기서 거래중으로 오판했다.'],
            'EST 월 08:00 KST = 일 18:00 ET → 거래중(재개 경계)'
                => ['2026-01-19 08:00', '거래중', 'EST 재개 경계는 월 07:00 이 아니라 월 08:00 KST 다.'],

            // ── 여름 EDT: 옛 하드코딩과 우연히 일치하던 구간(회귀 시 이 행은 침묵한다) ──
            'EDT 토 06:00 KST = 금 17:00 ET → 장마감(마감 경계)'
                => ['2026-07-18 06:00', '장마감', 'EDT 마감 경계는 토 06:00 KST — 옛 하드코딩과 우연히 일치했던 값.'],
            'EDT 월 07:00 KST = 일 18:00 ET → 거래중(재개 경계)'
                => ['2026-07-20 07:00', '거래중', 'EDT 재개 경계는 월 07:00 KST — 옛 하드코딩과 우연히 일치했던 값.'],
        ];
    }
     ...
     */
    public function testKrNightFuturesSession(string $kstLiteral, string $expected, string $why): void
    {
    ...
    public function krNightFuturesProvider(): array
    {
        return [
            // ── 새벽 소급(-1 day) ──────────────────────────────────────────
            '토 00:30 = 금 밤 세션의 새벽 연장 → 거래중'
                => ['2026-07-11 00:30', '거래중', '토 새벽은 전일(금 7/10) 저녁 세션의 연장이다. -1 day 소급이 빠지면 오늘=토=비거래일로 잘려 장마감으로 오판한다.'],
            '금 00:30 = 목 밤 세션의 새벽 연장 → 거래중'
                => ['2026-07-17 00:30', '거래중', '금 새벽은 전일(목 7/16) 저녁 세션의 연장 — 7/17 이 휴일이어도 전일 밤 세션은 살아 있다.'],
            '일 00:30 = 토 밤(비거래일) → 장마감'
                => ['2026-07-12 00:30', '장마감', '전일(토 7/11)이 비거래일이므로 소급할 세션 자체가 없다.'],

            // ── isKrTradingDay 연동 (제헌절 휴장) ─────────────────────────
            '목 18:00 (거래일) → 거래중'
                => ['2026-07-16 18:00', '거래중', 'KRX 공지 "7/16 18:00 개시분 정상".'],
            '금 18:00 = 제헌절 휴장 → 장마감'
                => ['2026-07-17 18:00', '장마감', 'KRX 공지 "7/17 18:00 개시분 휴장"(제헌절). isKrTradingDay 연동이 빠지면 휴일 밤에도 거래중으로 열린다.'],
            '토 00:30 (전일 7/17 휴장) → 장마감'
                => ['2026-07-18 00:30', '장마감', '전일(7/17)이 비거래일 → 소급 불가. 소급 규칙과 휴일 캘린더가 동시에 걸리는 케이스.'],
            '일 18:00 (비거래일) → 장마감'
                => ['2026-07-12 18:00', '장마감', '일요일은 비거래일 — 저녁 개시분 없음.'],

            // ── 시간 경계 ─────────────────────────────────────────────────
            '목 17:59 → 장마감(개시 전)'
                => ['2026-07-16 17:59', '장마감', '18:00 개시 전은 장마감(거래일이어도).'],
            '금 05:00 → 장마감(종료 경계)'
                => ['2026-07-17 05:00', '장마감', '새벽 연장은 05:00 에 끝난다 — 05:00 은 이미 장마감.'],
        ];
    }
}
